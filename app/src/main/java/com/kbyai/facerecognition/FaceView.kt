...
class FaceView(private var context: Context?, attrs: AttributeSet?) : View(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (frameSize != null && faceBoxes != null) {
            val x_scale =
                frameSize!!.width / canvas.width.toFloat()
            val y_scale =
                frameSize!!.height / canvas.height.toFloat()
            ...
            for (i in faceBoxes!!.indices) {
                ...
                if (faceBox.liveness < getLivenessThreshold(context!!)) {
                    ...
                    canvas.drawRect(
                        Rect(
                            (faceBox.x1 / x_scale).toInt(), (faceBox.y1 / y_scale).toInt(),
                            (faceBox.x2 / x_scale).toInt(), (faceBox.y2 / y_scale).toInt()
                        ), spoofPaint!!
                    )
                } else {
                    realPaint!!.strokeWidth = 3f
